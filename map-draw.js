...
export class MapDrawerMod extends AbstractMod {
    constructor() {
        // identifier, prettyName, loadPriority, dependencies
        super("map_drawer", "Map Drawer", 0, []);
    }
    ...
    initialize() {
                ...
                for(let dx=-1; dx<=1; dx++) {
                    for(let dy=-1; dy<=1; dy++) {
                        ...
                        if(nx >= 0 && nx < planet.config.width && ny >= 0 && ny < planet.config.height) {
                            ...
                            if (nc) { sum += nc.p[nx%cs][ny%cs]; count++; }
                        }
                    }
                }
        ...
        document.getElementById("dSav").addEventListener("click", () => {
            ...
            // Let GenTown's engine natively auto-calculate landmasses based on the custom map
                ...
                rd.onload = evt => {
                    try {
                        ...
                        if (d.width === planet.config.width && d.chunks) {
                            planet.chunks = d.chunks; rC();
                        } else { alert("Map config dimensions do not match."); }
                    } catch(err) { alert("Invalid map file format."); }
                };
                rd.readAsText(f);
            };
            inp.click();
...
// Automatically instantiate the mod to register it into $bml
new MapDrawerMod();
